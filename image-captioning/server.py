...
def preprocess(img):
    # Convert the image to size 299x299 as expected by the inception v3 model
...
if __name__ == "__main__":
    ...
    print("Sever started successfully")
    print("Waiting for request")

    # Init ImageHub for receive image from client
    ...
    while True:
        ...
        # Copy image for caption
        image_for_caption = image[:]

        # Preprocess image
        image_for_caption = cv2.resize(image_for_caption, (299, 299))
        image_for_caption = image_for_caption[..., ::-1]
        image_for_caption = encode(image_for_caption)
        image_for_caption = image_for_caption.reshape((1, 2048))

        # Get caption
        text = greedySearch(image_for_caption)
        print("Caption: ", text)

        # Reply caption back to client
        image_hub.send_reply(text)

        # Show image in server
